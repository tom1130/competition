...
def text_train(model, train_data, valid_data, lr, epochs, batch_size = 16, is_valid = True):
...
def image_train(model, train_data, valid_data, lr, epochs, batch_size = 16, is_valid = True):
    use_cuda, device = useCuda()
...
def image_valid(model, data, device, loss_func, batch_size = 16):
    dataloader = DataLoader(data, batch_size=batch_size, shuffle=True)
    total_val_accuracy = 0.0
    total_val_loss = 0.0
